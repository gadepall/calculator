...
to solve ordinary differential equations for trigonometric and exponential functions. This was chosen over alternative numerical integration techniques, keeping in mind the limited resources in a microcontroller based environment. A detailed analysis is given in Appendix~\ref{appendix:comparison}.
...
\end{align}
\begin{itemize}
  \item \textbf{Drawback:} For a step size of $h$, the error is $O(h)$. Thus to reduce error, we would have to increase step size drastically, leading to lots of computations.
\end{itemize}
\end{itemize}
...
\begin{itemize}
\begin{itemize}
\item \textbf{Drawbacks:} It is second order, meaning error of $O(h^2)$. It is also implicit, meaning each step would require another iterative solving mechanism such as Newton-Rhapson.
\end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \end{align*}
    \begin{itemize}
    \item \textbf{Drawback:} Both methods are second-order, with global error proportional to $h^2$ (denoted $O(h^2)$). While this is a significant improvement over Euler's method, it is still much less accurate than the fourth-order RK4 method ($O(h^4)$). Given that RK4 only requires twice the number of function evaluations per step as these RK2 methods (four vs. two), the substantial gain in accuracy is a highly favorable trade-off for a scientific calculator application.
\end{itemize}
\end{itemize}

...
\item 
	The fast inverse square root algorithm (Quake III method) is used to efficiently compute $1/\sqrt{x}$ which is then used for computing $\sqrt{x}$. 
	%See 
...
\figref{fig:circuit_schematic}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/schematic.jpg}
...
\iffalse
\subsection{Quake III Algorithm}

\label{appendix:inv_sqrt}

%The method obtains an initial approximation by manipulating the IEEE~754 floating-point representation of $x$, then refines it using Newton--Raphson iterations\cite{lomont2003}.
%
%\begin{enumerate}
...
\end{align}
Applying the Newton-Raphson method \cite{grewal2014},
\begin{align}
...
\begin{align}
y \;\gets\; y_0 \left( 1.5 - 0.5 \, x \, y_0^2 \right).
\end{align}
\fi
